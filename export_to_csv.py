...
def parse_ts_file(file_path):
    ...
    # Very basic parser for the object literals
    # We look for each object {...}
    tools = []
    
    # Find all top-level objects in the array
    # This is a bit naive but should work for this specific structure
    # We find { followed by slug: "..." and then up to },
    ...
    for match in obj_pattern.finditer(tools_content):
        ...
        def get_field(field, text):
            # Matches field: "value" or field: true/false or field: ["val"]
            m = re.search(rf'{field}:\s*(.*?),\n', text)
            if not m: return ""
            val = m.group(1).strip()
            # Clean up quotes if it's a string
            if val.startswith('"') and val.endswith('"'):
                return val[1:-1]
            return val

        # Helper to extract integrations (more complex)
        def get_integrations(text):
            m = re.search(r'integrations:\s*\[(.*?)\]', text, re.DOTALL)
            if not m: return "[]"
            # Return as a string for now, user can manage JSON in sheet or we can flatten
...
if tools:
    ...
    print(f"Exported {len(tools)} tools to tools_export.csv")
else:
    print("No tools found to export.")
