...
async function getToolDetails(toolId: string): Promise<ToolRecord | null> {
  try {
    console.log(`[ToolPage] Attempting to fetch tool details for ID: ${toolId}`);
    const record = await base("Tools").find(toolId);

    if (!record) {
      console.warn(`[ToolPage] Tool with ID ${toolId} not found in Airtable.`);
      return null;
    }
    ...
    console.log(`[ToolPage] Raw fields for tool ${tool.fields.Name}:`, JSON.stringify(tool.fields, null, 2));
    return tool;
  } catch (error) {
    console.error(`[ToolPage] Error fetching tool ${toolId}:`, error);
    if (error instanceof Error) {
      console.error(`[ToolPage] Error message: ${error.message}`);
      console.error(`[ToolPage] Error stack: ${error.stack}`);
    }
    return null;
  }
}
...
export default async function ToolDetailPage({ params }: any) {
  ...
  console.log(`[ToolPage] Rendering ToolDetailPage for ID: ${id}`);

  const tool = await getToolDetails(id); // Use the destructured id

  if (!tool) {
    console.log(`[ToolPage] Tool not found for ID ${id}, rendering "Tool Not Found" message.`);
    return (
  ...
  console.log(`[ToolPage] Images array for ${name}:`, images); // This log should now appear if tool is found
