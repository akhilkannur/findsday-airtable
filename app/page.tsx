...
let latestDropRecordId: string | null = null; // Explicitly type as string | null
...
if (dropsRecords.length > 0) {
  ...
  if (typeof potentialDropNumber === "number" && !isNaN(potentialDropNumber)) {
    ...
    latestDropRecordId = dropsRecords[0].id; // This is the record ID of the drop
    console.log(`[DEBUG] Latest Drop found: Drop Number ${latestDropNumber}, Record ID: ${latestDropRecordId}`);
  } else {
    console.warn("[WARN] Latest drop record found, but 'Drop Number' field is not a valid number.");
  }
} else {
  console.warn("[WARN] No 'Drops' records found in Airtable. Defaulting to Drop Number 1.");
}

// --- Fetch Tools linked to the Latest Drop Record ---
let tools: ToolRecord[] = []; // Initialize tools as an empty array of ToolRecord
if (latestDropRecordId) {
  console.log(`[DEBUG] Attempting to fetch tools for Drop Record ID: ${latestDropRecordId}`);
  ...
  // The filter uses the record ID of the Drop to find linked tools.
  ...
  tools = fetchedToolsRecords.map((record) => ({
    id: record.id,
    fields: record.fields,
  })) as ToolRecord[];
  
  console.log(`[DEBUG] Fetched ${tools.length} tools for Drop Record ID: ${latestDropRecordId}`);
  if (tools.length === 0) {
    console.log("[INFO] No tools found linked to the latest drop. Please ensure tools are linked to the latest drop in Airtable via the 'Drop' field.");
  }
} else {
  console.log("[INFO] No latest drop record ID available, skipping tool fetch for drops.");
}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {sponsors.map((sponsor) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="bg-charcoal-dark border border-gray-800 rounded-lg p-6 flex flex-col md:flex-row items-center gap-6 cursor-pointer hover:shadow-lg transition-shadow duration-200"
            >
              {sponsor.fields.Logo?.[0] ? (
                ...
                <div className="w-24 h-24 bg-gray-700 flex items-center justify-center text-gray-400 text-sm font-bold rounded-lg flex-shrink-0">
                  NO LOGO
                </div>
              )}
              <div>
                <h3 className="text-2xl font-bold text-white mb-2">{sponsor.fields.Name || "Untitled Sponsor"}</h3>
